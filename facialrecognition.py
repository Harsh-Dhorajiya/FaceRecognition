...
data_path = 'E:/Study/openCV/faces/'
...
model.train(np.asarray(Training_dataset) , np.asarray(labels))

print("Model is training successfully ")
...
def face_detector(img, size= 0.5):
    ...
    for(x,y,w,h) in faces:
        ...
        roi = img[y:y+h, x:x+w]
        new_roi = cv2.resize(roi, (200,200))
...
while True:
    ...
    try:
        ...
        if result[1] < 500:
            ...
            display_string = str(confidence) + '% Confidence is user'
        cv2.putText(image,display_string,(100,120),cv2.FONT_HERSHEY_COMPLEX,1,(250,120,255),2)


        if confidence > 75:
            cv2.putText(image, "UNLOCKED", (250, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255),2)
            cv2.putText(image, "HARSH", (245, 220), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255),2)
